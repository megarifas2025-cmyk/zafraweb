...
const EMERALD_RING = 'rgba(4, 120, 87, 0.25)';
...
/** Cabecera identidad comprador — alineada a `diseños/perfil comprador.txt` */
export function BuyerIdentityHeader({ perfil, isVerificado, onBell, onLogout, showNotificationDot = false }: Props) {
  const insets = useSafeAreaInsets();
  const nombre = perfil?.nombre?.trim() || 'Comprador';
  const muni = perfil?.municipio?.trim();
  const estado = perfil?.estado_ve?.trim();
  const loc = [muni, estado].filter(Boolean).join(', ') || 'Venezuela';
    ...
    <View style={[s.wrap, { paddingTop: Math.max(insets.top, 12) }]}>
      <View style={s.left}>
        ...
        <View style={s.textCol}>
          ...
          <View style={s.locRow}>
            <Ionicons name="location-outline" size={12} color={EMERALD} />
